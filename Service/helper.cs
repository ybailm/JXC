...
{    /// <summary>    
    ...
    public static class helper
    {
        ...
        public static string connectionString = ConfigurationManager.AppSettings["OleDbConnectionstring"];
        ...
        public static int GetMaxID(string FieldName, string TableName)
        {
            ...
            object obj = helper.GetSingle(strsql);
            if (obj == null)
            {
                return 1;
            }
        ...
        {
            object obj = helper.GetSingle(strSql, cmdParms);
            int cmdresult;
            if ((Object.Equals(obj, null)) || (Object.Equals(obj, System.DBNull.Value)))
            {
                cmdresult = 0;
        ...
        public static DataTable GetTable(string tableName)
        {
            using (OleDbConnection connection = new OleDbConnection(connectionString))
            {
                ...
                {
                    connection.Open();
                    OleDbDataAdapter command = new OleDbDataAdapter($"Select * From {tableName}", connection);
                    command.Fill(dt);
                }
        ...
        public static DataTable GetTable(string tableName, string condition)
        {
            using (OleDbConnection connection = new OleDbConnection(connectionString))
            {
                ...
                {
                    connection.Open();
                    OleDbDataAdapter command = new OleDbDataAdapter($"Select * From {tableName} Where {condition}", connection);
                    command.Fill(dt);
                }
                catch (OleDbException ex)
                {
        ...
        public static DataTable GetColumnMaxValue(string tableName, string columnName)
        {
        ...
        /// <param name="SQLString">SQL语句</param>    
        /// <returns>影响的记录数</returns>    
        public static int ExecuteSqlReturnInt(string SQLString, params OleDbParameter[] cmdParms)
        {
            using (OleDbConnection connection = new OleDbConnection(connectionString))
            {
        ...
        public static bool ExecuteTransaction(List<TransactionModel<OleDbParameter>> transList)
        {
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                conn.Open();
                using (OleDbTransaction trans = conn.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        {
                            PrepareCommand(cmd, conn, trans, item.SQL, item.Pars);
                            int val = cmd.ExecuteNonQuery();
                            cmd.Parameters.Clear();
                        }
                        trans.Commit();
